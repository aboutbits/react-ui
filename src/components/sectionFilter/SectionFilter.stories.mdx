...
import { SectionFilterDialogAction } from './SectionFilterDialogAction'
...
export const Template = () => {
  ...
  const showFilterAsDialog = useMatchMediaQuery('(max-width: 1024px)')
    ...
    <Section>
      <SectionHeader>
        ...
        <SectionFilter
          ...
          asDialog={showFilterAsDialog}
          dialogShow={filterDialogShow}
          dialogProps={{
            confirmationButtonContent: 'Apply filter',
            onDismiss: () => setFilterDialogShow(false),
          }}
        >
...
export const TemplateWithoutDialog = () => {
  const [filter, setFilter] = useState({ role: '', department: '' })
  const content = Array.from(Array(10).keys())
    .map((item, index) => ({
      name: `User ${item + 1}`,
...
<Canvas>
  <Story name="WithoutDialog">{TemplateWithoutDialog.bind({})}</Story>
</Canvas>
