...
<Meta
  ...
  decorators={[
    (Story) => {
      return (
        <Formik initialValues={{ active: false }} onSubmit={action('onSubmit')}>
          <Form>
            <Story />
          </Form>
        </Formik>
...
This component can be used to create toggle switches. This input already has a label and handles error messages in `formik` Forms.
